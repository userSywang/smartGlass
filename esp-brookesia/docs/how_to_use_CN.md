...
用户点击图标
```c
  -> Core.sendAppEvent(START)
    -> CoreManager.onAppEventCallback()
      -> CoreManager.startApp(id)
        -> CoreManager.processAppRun(app)
          -> PhoneHome.processAppRun(app)
               - 状态栏/导航栏可见性
               - 最近任务快照配置与添加
          -> CoreApp.processRun()
               - startRecordResource()
               - (可选) initDefaultScreen()
               - App.run()
               - endRecordResource()
          -> active_app = app
```
**[切到其他 App/主页]**
```c
CoreManager.processAppPause(active_app)
  -> CoreApp.processPause()
       - App.pause()
       - saveAppTheme / saveRecentScreen / loadDisplayTheme
  -> (若开快照) saveAppSnapshot()
  -> PhoneHome.processAppPause(app)
  -> active_app 仍指向旧 app，但 UI 已切走

**[回到 App]**
CoreManager.processAppResume(app)
  -> (若当前有别的 active_app) 先 processAppPause(old)
  -> PhoneHome.processAppResume(app)
       - 恢复状态栏/导航栏可见性
  -> CoreApp.processResume()
       - loadRecentScreen()
       - loadAppTheme()
       - startRecordResource()
       - App.resume()
...
**[进入最近任务]**
```c
Core.sendNavigateEvent(RECENTS_SCREEN)
  -> Core.onCoreNavigateEventCallback()
    -> (系统侧通常触发) PhoneHome.processRecentsScreenShow()
         - 调整状态栏/导航栏显示
         - _recents_screen->setVisible(true)
    -> RecentsScreen 使用 snapshot 展示各 app
```
**[返回并关闭 App]**
```c
App.back()
  -> CoreApp.notifyCoreClosed()
    -> Core.sendAppEvent(STOP)
      -> CoreManager.processAppClose(app)
        -> CoreApp.processClose(is_active)
             - App.close()
             - (active) enableAutoClean 等待屏幕卸载回收
        -> PhoneHome.processAppClose(app)
             - 移除状态栏图标
             - 移除最近任务快照
        -> 从 running map 移除，active_app 清空
 ```